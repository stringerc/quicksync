...
// Configuration
// -----------------------------------------------------------------------------
const configPath = process.env.RESONANCE_CONFIG_FILE ||
...
const baseSecurityConfig = {
    ...
    rateLimitWindowMs: parseInt(process.env.RESONANCE_RATE_LIMIT_WINDOW_MS || '60000', 10),
    rateLimitMaxRequests: parseInt(process.env.RESONANCE_RATE_LIMIT_MAX || '100', 10),
...
const intakeSecurityConfig = {
    ...baseSecurityConfig,
    apiKey: process.env.RESONANCE_INTAKE_API_KEY || baseSecurityConfig.apiKey,
    rateLimitWindowMs: parseInt(process.env.RESONANCE_INTAKE_RATE_LIMIT_WINDOW_MS || '60000', 10),
    rateLimitMaxRequests: parseInt(process.env.RESONANCE_INTAKE_RATE_LIMIT_MAX || '600', 10),
};
...
(0, updater_1.handleCliUpdateCommand)({ stagingDir: process.env.RESONANCE_UPDATE_STAGING_DIR });
// -----------------------------------------------------------------------------
// Core initialisation
// -----------------------------------------------------------------------------
...
function withSecurity(handler, config) {
    return (req, res) => {
        (0, security_1.setSecurityHeaders)(res, config);
        ...
        if (!(0, security_1.validateApiKey)(req, config)) {
            res.writeHead(401, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: 'Unauthorized: Invalid or missing API key' }));
            return;
        }
        const clientIP = (0, security_1.getClientIP)(req);
        if (!(0, security_1.checkRateLimit)(clientIP, config)) {
...
const handlePhaseIntake = (req, res) => {
    let body = '';
    req.on('data', (chunk) => {
        body += chunk;
        if (body.length > 1e6) {
    ...
    req.on('end', () => {
        try {
            ...
            const state = core.getState();
            const spectralEntropy = typeof payload.spectralEntropy === 'number'
                ? payload.spectralEntropy
                : state.spectralEntropy;
            const p99Risk = typeof payload.p99Risk === 'number' ? payload.p99Risk : payload.tailRisk ?? 0.1;
...
const secureIntakeHandler = intakeSecurityConfig.apiKey && intakeSecurityConfig.apiKey.length > 0
...
const server = http.createServer((req, res) => {
    try {
        ...
        (0, security_1.setSecurityHeaders)(res, baseSecurityConfig);
...
server.listen(port, '0.0.0.0', () => {
    ...
    (0, updater_1.scheduleAutoUpdate)({
