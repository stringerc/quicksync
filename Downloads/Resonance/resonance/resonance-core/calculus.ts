...
export interface ArrivalServiceCurves {
  arrival: number[];  // Arrival count per time window
  service: number[];  // Service completion count per time window
}
 ...
 */
export function maxPlusConvolve(alpha: number[], beta: number[]): number[] {
  const result: number[] = [];
 ...
 */
export function minPlusDeconvolve(alpha: number[], beta: number[]): number[] {
  const result: number[] = [];
 ...
 */
export class ResonanceCalculus {
   ...
   */
  private slidingWindow(windowMs: number, timestamps: number[]): number[] {
    const now = Date.now();
   ...
   */
  decide(deferBudgetMs: number, hedgeThresholdMs = 100): CalculusDecision {
